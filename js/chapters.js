...
jQuery( document ).ready( function() {
	...
	tocContainer.find( "> ul .menu-item-has-children > a" ).wrap( '<div class="expandable"></div>' ).after( '<button class="dashicons dashicons-arrow-down-alt2" aria-expanded="false"></button>' );
	// Invisibly hide all of the submenus
	jQuery( ".menu-item-has-children > ul ul" ).hide();
	// Open the current menu
	tocContainer.find( ".current-menu-item a" ).first().addClass( "active" ).parents( ".menu-item-has-children" ).toggleClass( "open" ).find( "> div > .dashicons" ).attr( "aria-expanded", true );
	...
	jQuery( ".menu-item-has-children > div > .dashicons" ).click( function() {
		...
		jQuery( this ).parent().siblings( ".sub-menu" ).length ? jQuery( this ).parent().siblings( ".sub-menu" ).slideToggle() : jQuery( this ).parent().siblings( ".children" ).slideToggle();
