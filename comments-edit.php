 ...
 *
 * NOTE: This template overrides original in order to prevent passing parameters
 * 'logged_in_as' and 'must_log_in' to comment_form() function. These parameters
 * contain links to login.wordpress.org and can't be changed other way.
 *
 * @package RefPress
 */
...
$ref_url     = DevHub\get_site_section_url();
$ref_link    = sprintf( '<a href="%s">%s</a>', esc_url( $ref_url ), __( 'Reference', 'refpress' ) );
$post_link   = sprintf( '<a href="%s">%s</a>', esc_url( $post_url ), $post_title );
/* translators: %d: comment ID */
$note_link   = sprintf( '<a href="%s">%s</a>', esc_url( $post_url . '#comment-' . $comment_id ), sprintf( __( 'note %d', 'refpress' ), $comment_id ) );
$type_link   = sprintf( '<a href="%s">%s</a>', esc_url( $type_url ), $type_label );

$parent_link   = '';
$parent_author = '';
if ( $is_parent && isset( $parent->comment_ID ) ) {
	$parent_author = get_comment_author_link( $parent );
	/* translators: %d: comment ID */
	$parent_label  = sprintf( __( 'note %d', 'refpress' ), $parent->comment_ID );
...
?>
	...
	<div id="content-area" <?php body_class( 'code-reference' ); ?>>
		...
		<main id="main" class="site-main" role="main">
			<h1><?php printf( __( 'Edit Note %d', 'refpress' ), $comment_id ); ?></h1>

			<p>
				<?php if ( $is_parent ) : ?>
					<?php
						/* translators: 1: comment title, 2: comment author name, 3: reference type (function, class, method, hook), 4: post title */
						printf( __( 'This is a feedback note to %1$s by %2$s for the %3$s %4$s.', 'refpress' ), $parent_link, $parent_author, strtolower( $type_single ), $post_link );
					?>
				<?php else : ?>
					<?php
						/* translators: 1: reference type (function, class, method, hook), 2: post title */
						printf( __( 'This is a note for the %1$s %2$s.', 'refpress' ), strtolower( $type_single ), $post_link ); ?>
				<?php endif; ?>

				<?php echo ' ' . __( "You can edit this note as long as it's in moderation.", 'refpress' ); ?>
			</p>
			<?php
				if ( $is_parent ) {
					echo refpress_compat_wp_editor_feedback( $comment, 'show', true );
				} else {
					$args = \DevHub_User_Submitted_Content::comment_form_args( $comment, 'edit');
					unset( $args['logged_in_as'] );
					unset( $args['must_log_in'] );
					comment_form( $args );
				}
			?>
